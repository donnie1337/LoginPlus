...
public final class MessagesManager {
    ...
    public MessagesManager(JavaPlugin plugin) {
        ...
        if (!titlesFile.exists()) plugin.saveResource("mensagens/titulos.yml", false);
        this.titles = YamlConfiguration.loadConfiguration(titlesFile);

        File chatFile = new File(plugin.getDataFolder(), "mensagens/chat.yml");
        if (!chatFile.exists()) plugin.saveResource("mensagens/chat.yml", false);
        this.chat = YamlConfiguration.loadConfiguration(chatFile);
    }

    public String getTitleBemVindo() { return color(titles.getString("bem-vindo", "&a&lBEM-VINDO!")); }
    public String getTitleRegistro() { return color(titles.getString("registro", "&eUse: /registro <senha> <senha>")); }
    public String getTitleLogin() { return color(titles.getString("login", "&eUse: /login <senha>")); }
    public String getTitlePremium() { return color(titles.getString("premium", "&eConta original detectada.")); }
